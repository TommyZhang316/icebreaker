          ...
          <div className="bg-white/5 rounded-xl p-4">
            ...
            <div className="flex flex-wrap gap-2 justify-center">
              {gs.participants.map((p) => (
                <span
                  key={p.nickname}
                  className={p.nickname === me ? 'pill my-pill' : 'pill other-pill'}
                >
                  {p.nickname}
                </span>
              ))}
    ...
    return (
      <main className="game-bg flex flex-col items-center justify-center p-4 gap-4">
        ...
        <p className="text-white/40 text-sm">
          第 {gs.currentQuestionIndex + 1} / {gs.totalQuestions} 题
        </p>
        ...
        <div className="glass w-full max-w-xl p-6">
          ...
          {/* Not answered yet: show option buttons */}
          {myAns === undefined ? (
            ...
            /* Already answered: show mini live result */
            <div>
              {/* My selection badge */}
              <div className="flex justify-center mb-5">
                <span
                  className="px-5 py-2 rounded-full text-white font-bold text-base"
  ...
  if (gs.status === 'results') {
    ...
    return (
      <main className="game-bg flex flex-col items-center justify-center p-4 gap-4">
        ...
        <div className="glass w-full max-w-xl p-6">
          <h2 className="text-base font-semibold text-white/70 text-center mb-5">
            {q.text}
          </h2>
                ...
                <div key={opt}>
                  ...
                  >
                    {names.map((name) => (
                      <span
                        key={name}
                        className={name === me ? 'pill my-pill' : 'pill other-pill'}
                      >
                        {name === me ? `✨ ${name}` : name}
                      </span>
                    ))}
    ...
    return (
      <main className="game-bg flex items-center justify-center p-4">
        <div className="glass w-full max-w-sm p-8 text-center">
          <div className="text-5xl mb-4" style={{ animation: 'pulse-slow 2s infinite' }}>🎲</div>
